...
func NewHttpsClient(certs tls.Certificate) *http.Client {
	...
	tr = &http.Transport{
		...
		TLSClientConfig: &tls.Config{RootCAs: pool, InsecureSkipVerify: true, Certificates: []tls.Certificate{certs}},
		//TLSClientConfig: &tls.Config{InsecureSkipVerify: true},
	}
	return &http.Client{Transport: tr}
}
...
func ClientPost(c *http.Client, url, content string) ([]byte, error) {
	resp, err := c.Post(url, "application/x-www-form-urlencoded", strings.NewReader(content))
	if err != nil {
		return nil, err
	}
	defer resp.Body.Close()
